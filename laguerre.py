...
def display(S, R, tri_list, voronoi_cell_map, xbounds, ybounds, radius, zShift ):
	# Setup
	...
	circle = plot.Circle((0., 0.), radius, color='r', fill=False )
	ax.add_patch(circle)

	plot.xlim((xbounds[0],xbounds[1]))
	plot.annotate('z = '+str(zShift), (1.2,-1.6), size=25, color='indigo', \
	bbox=dict(boxstyle='round,pad=0.2', fc='aquamarine', alpha=1.0))
	#plot.ylim((ybounds[0],ybounds[1]))
	plot.xlabel(r'$x~(R_{J})$',fontsize=15)
	plot.ylabel(r'$y~(R_{J})$',fontsize=15)
